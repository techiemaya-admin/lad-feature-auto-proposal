...
7. Add-ons the lead picks: table kind "addons" + a multi_choice input + aggregate (sum/count, rows: "selected", selected_var, key_column) + a rows variable for the document's loop.
   ...
   * Keeping it out of the table also keeps it out of any COUNT of selected add-ons, which is what a "2+ add-ons bundled" discount counts. If the notes leave it open whether such an extra counts toward that threshold, or whether the discount comes off its fee too, decide, and say which way in assumptions[].
...
9. Taxes. First decide from the notes whether tax depends on WHERE THE BUYER IS.
   * It does not apply at all (the notes say the agency does not charge it, or are unsure and the SAMPLE QUOTATION shows no tax): define NO tax variables whatsoever — no rate, no flag, no amount. A tax section the document has no tag for is one you invented, and a $0.00 tax line with an always-true flag beside it reads to the drafter as "tax applies" and puts that sentence in front of the client. Say what you concluded in assumptions[] instead.
