...
export interface AiCell { column: string; text: string }
export interface AiTable { id: string; label: string; kind: string; columns: { key: string; label: string; unit: string }[]; rows: { cells: AiCell[] }[] }
export interface AiWhere { column: string; op: string; value_var: string; value: string; values: string[] }
export interface AiCond { var: string; op: string; value_var: string; value: string; values: string[] }
export interface AiMapEntry { loop_column: string; column: string; op: string; args: string[] }
export interface AiVariable {
  name: string; label: string; in_document: boolean; unit: string; condition_flag: string; kind: string;
  input_type: string; options_table: string; options_column: string; options: string[]; required: boolean;
  value: string;
  table: string; where: AiWhere[]; take: string;
  op: string; args: string[];
  all: AiCond[];
  fn: string; rows: string; selected_var: string; key_column: string; column: string;
  map: AiMapEntry[];
}
...
export function parseCell(text: unknown, unit: string): Cell {
  if (typeof text === "number" || typeof text === "boolean") {
    ...
    if (unit === "boolean") return typeof text === "boolean" ? text : text !== 0;
...
const parseArg = (a: unknown): string | number => {
  ...
  return s !== "" && !Number.isNaN(Number(s)) && !/^col:/.test(s) ? Number(s) : s;
};
...
const parseLiteral = (s: unknown): Cell => {
  ...
  return Number.isNaN(n) ? t : /%$/.test(t) ? Number(t.replace(/[%,\s]/g, "")) / 100 : n;
};

const fromWireCond = <K extends "column" | "var">(key: K, c: AiWhere | AiCond): any => {
...
export function fromWire(w: AiPricingRules): PricingRules {
  const tables: RuleTable[] = (w.tables ?? []).map((t) => {
    const columns = (t.columns ?? []).map((c) => ({ key: c.key, label: c.label ?? c.key, unit: c.unit as CellUnit }));
    const unitOf = new Map(columns.map((c) => [c.key, c.unit as string]));
    return {
      id: t.id, label: t.label ?? t.id, kind: t.kind as TableKind, columns,
      rows: (t.rows ?? []).map((r) => {
        const row: Row = {};
        for (const c of columns) row[c.key] = null;
        for (const cell of r.cells ?? []) row[cell.column] = parseCell(cell.text, unitOf.get(cell.column) ?? "text");
        return row;
      }),
    };
  });

  const variables: RuleVariable[] = (w.variables ?? []).map((v): RuleVariable => {
    const base = { name: v.name, label: v.label ?? v.name, in_document: Boolean(v.in_document), unit: v.unit as Unit, condition_flag: v.condition_flag ?? "" };
    const where = () => (v.where ?? []).map((c) => fromWireCond("column", c) as Where);
    const sel = () => ({ rows: (v.rows as "selected" | "all") ?? "all", ...(isBlank(v.selected_var) ? {} : { selected_var: v.selected_var }), key_column: v.key_column ?? "", ...(v.where?.length ? { where: where() } : {}) });
    switch (v.kind) {
      case "input":
        return { ...base, kind: "input", input_type: v.input_type as InputType, required: Boolean(v.required),
          ...(isBlank(v.options_table) ? {} : { options_table: v.options_table }),
          ...(isBlank(v.options_column) ? {} : { options_column: v.options_column }),
          ...(v.options?.length ? { options: v.options } : {}) };
      case "constant":
        return { ...base, kind: "constant", value: parseCell(v.value, v.unit) };
      case "lookup":
        return { ...base, kind: "lookup", table: v.table ?? "", where: where(), take: v.take ?? "" };
      case "formula":
        return { ...base, kind: "formula", op: v.op as FormulaOp, args: (v.args ?? []).map(parseArg) };
      case "condition":
        return { ...base, kind: "condition", all: (v.all ?? []).map((c) => fromWireCond("var", c) as Cond) };
      case "aggregate":
        return { ...base, kind: "aggregate", fn: v.fn as "sum" | "count", table: v.table ?? "", ...sel(), ...(isBlank(v.column) ? {} : { column: v.column }) };
      case "rows": {
        const map: Record<string, string | Formula> = {};
        for (const m of v.map ?? []) map[m.loop_column] = isBlank(m.op) ? m.column : { op: m.op as FormulaOp, args: (m.args ?? []).map(parseArg) };
  ...
  for (const s of w.sample_inputs ?? []) sample_inputs[s.name] = fromWireValue(s);

  return {
    version: 1, tables, variables,
    review_rules: (w.review_rules ?? []).map((r) => ({ when: (r.when ?? []).map((c) => fromWireCond("var", c) as Cond), reason: r.reason })),
    assumptions: (w.assumptions ?? []).map((a) => ({ text: a.text ?? "", resolved_as: a.resolved_as ?? "" })),
    sample_inputs,
  };
}
...
const toWireCond = (c: Where | Cond): AiWhere & AiCond => ({
  column: (c as Where).column ?? "", var: (c as Cond).var ?? "", op: c.op,
  value_var: c.value_var ?? "", value: c.value === undefined ? "" : cellText(c.value), values: (c.values ?? []).map(cellText),
});

const EMPTY_VAR: Omit<AiVariable, "name" | "label" | "in_document" | "unit" | "condition_flag" | "kind"> = {
  input_type: "", options_table: "", options_column: "", options: [], required: false, value: "",
  table: "", where: [], take: "", op: "", args: [], all: [], fn: "", rows: "", selected_var: "", key_column: "", column: "", map: [],
};
...
export function toWire(r: PricingRules): AiPricingRules {
  return {
    ...
    tables: r.tables.map((t) => ({
      id: t.id, label: t.label, kind: t.kind, columns: t.columns.map((c) => ({ ...c })),
      rows: t.rows.map((row) => ({ cells: t.columns.map((c) => ({ column: c.key, text: cellText(row[c.key] ?? null) })) })),
    })),
    variables: r.variables.map((v): AiVariable => {
      const out: AiVariable = { ...EMPTY_VAR, name: v.name, label: v.label, in_document: v.in_document, unit: v.unit, condition_flag: v.condition_flag, kind: v.kind };
      switch (v.kind) {
        case "input": Object.assign(out, { input_type: v.input_type, options_table: v.options_table ?? "", options_column: v.options_column ?? "", options: v.options ?? [], required: v.required }); break;
        case "constant": out.value = cellText(v.value); break;
        case "lookup": Object.assign(out, { table: v.table, where: v.where.map(toWireCond), take: v.take }); break;
        case "formula": Object.assign(out, { op: v.op, args: v.args.map(String) }); break;
        case "condition": out.all = v.all.map(toWireCond); break;
        case "aggregate": Object.assign(out, { fn: v.fn, table: v.table, rows: v.rows, selected_var: v.selected_var ?? "", key_column: v.key_column, column: v.column ?? "", where: (v.where ?? []).map(toWireCond) }); break;
        case "rows": Object.assign(out, { table: v.table, rows: v.rows, selected_var: v.selected_var ?? "", key_column: v.key_column, where: (v.where ?? []).map(toWireCond),
          map: Object.entries(v.map).map(([loop_column, m]) => typeof m === "string" ? { loop_column, column: m, op: "", args: [] } : { loop_column, column: "", op: m.op, args: m.args.map(String) }) }); break;
      }
      return out;
    }),
    review_rules: r.review_rules.map((x) => ({ when: x.when.map(toWireCond), reason: x.reason })),
    assumptions: r.assumptions.map((a) => ({ ...a })),
    sample_inputs: Object.entries(r.sample_inputs).map(([name, v]) => Array.isArray(v) ? { name, value: "", values: (v as unknown[]).map(String) } : { name, value: String(v), values: [] }),
  };
}
