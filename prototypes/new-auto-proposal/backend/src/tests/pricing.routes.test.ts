...
test("Pricing rules routes", async (t) => {
  ...
  const getRules = () => request(app).get("/api/companies/co1_seo/templates/default-co1_seo/rules");
  const stageOf = async () => (await request(app).get("/api/companies/co1_seo/templates/default-co1_seo")).body.company.working_state?.stage;
  ...
  await t.test("404 before compile, then compile persists an all-green state and moves the stage", async () => {
    ...
    const res = await request(app).post("/api/companies/co1_seo/templates/default-co1_seo/rules/compile");
  ...
  await t.test("PUT rejects structural errors without persisting, accepts edits and re-checks the sample", async () => {
    ...
    const rejected = await request(app).put("/api/companies/co1_seo/templates/default-co1_seo/rules").send({ rules: bad });
    ...
    const accepted = await request(app).put("/api/companies/co1_seo/templates/default-co1_seo/rules").send({ rules: edited });
  ...
  await t.test("calculate evaluates arbitrary lead inputs and returns the proposal payload", async () => {
    const res = await request(app).post("/api/companies/co1_seo/templates/default-co1_seo/rules/calculate").send({ inputs: { location_count: 3, client_state: "CA" } });
    assert.equal(res.status, 200, res.text);
  ...
  await t.test("proceed moves to lead_simulation only when the rules are valid", async () => {
    const res = await request(app).post("/api/companies/co1_seo/templates/default-co1_seo/rules/proceed");
    assert.equal(res.status, 200, res.text);
  ...
  await t.test("regenerating the template and unlocking the briefing both reset Stage 4", async () => {
    const regen = await request(app).post("/api/companies/co1_seo/templates/default-co1_seo/template/generate");
    assert.equal(regen.status, 200, regen.text);
    assert.equal((await getRules()).status, 404);
    assert.equal((await request(app).post("/api/companies/co1_seo/templates/default-co1_seo/rules/proceed")).status, 409);
    ...
    const unlock = await request(app).post("/api/companies/co1_seo/templates/default-co1_seo/briefing/unlock");
  ...
  await t.test("compile refuses a company that has not reached Stage 2", async () => {
    const res = await request(app).post("/api/companies/co2_msp/templates/default-co2_msp/rules/compile");
